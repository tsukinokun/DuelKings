...
public:
    ...
    //---------------------------------------------------------------------------
    // 文字の太さを設定
    //! @param thick_size [in] 太さ(0 ~ 9の間の値で)
    //! @retval 自身のポインタ
    //---------------------------------------------------------------------------
    std::shared_ptr<ComponentText> SetThickSize(int thick_size);

private:
    //---------------------------------------------------------------------------
    // テキストの折り返し処理
    //! @param max_width [in] 折り返しを行う最大幅
    //! @param font_handle [in] フォントハンドル
    //! @retval 折り返し後の文字列
    //---------------------------------------------------------------------------
    std::string WrapText(int font_handle);
...
private:
    ...
    int         max_width_  = 100000;               //折り返しを行う最大幅
